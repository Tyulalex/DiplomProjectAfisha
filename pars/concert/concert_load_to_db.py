...
if __name__ == '__main__':
    ...
    main_func.get_events_url_file(action_type_id) # файл - concert_urls.txt
    main_func.get_events_url_file(action_type_id) # файл - theatre_urls.txt
    ...
    with open('concert_urls.txt' , 'r') as f:
        for line in f:
            ...
            if url:
                bs = (main_func.fetch_content(url),'html.parser')
                concert_dict = spec_func.fetch_event_info(bs)
    ...
    with open('theatre_urls.txt' , 'r') as f:
        for line in f:
            ...
            if url:
                bs = (main_func.fetch_content(url),'html.parser')
                theatre_dict = spec_func.fetch_event_info(bs)
